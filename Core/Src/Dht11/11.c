...
/*------------------------------*/
void DHT_GPIO_SET_OUTPUT(
    void) // 设置GPIOx为输出模式（MCU的IO口向DHT11发激活信号）
{
  GPIO_InitTypeDef
      GPIO_InitStructure; // 在GPIO_InitTypeDef结构体中修改IO口参数（结构体成员）
  GPIO_InitStructure.Pin =
      GPIO_PIN_10; // 设置的格式必须严格遵循注释，比如GPIO_PIN_define
...
void DHT_GPIO_SET_INPUT(
    void) // 设置GPIOx为输入模式（DHT11向MUC的IO发电平信号，信号里包含了温湿度信息）
{
...
{
  uint8_t ReadData =
      0;             // ReadData用于存放8bit数据，即8个单次读取的1bit数据的组合
  uint8_t temp;      // 临时存放信号电平（0或1）
  uint8_t retry = 0; // retry用于防止卡死
  uint8_t i;
  for (i = 0; i < 8; i++) // 一次温湿度信号读取八位
  {
    while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == 0 && retry < 100)
    ...
    while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == 1 && retry < 100)
  ...
  // char str[50];
  // sprintf(str, "%d--", ReadData);
  // UART2_TransmitString_DMA(str);
...
uint8_t DHT_Read(void) {
  ...
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10,
                    GPIO_PIN_RESET); // IO->DHT11:先拉低电平18ms（应时序要求）
  delay_ms(18);
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10,
                    GPIO_PIN_SET); // IO->DHT11:随后拉高电平20us
  delay_us(20);
  ...
  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) ==
      0) // DHT11发回低电平响应（读取到低电平，说明DHT11有响应）
  {
